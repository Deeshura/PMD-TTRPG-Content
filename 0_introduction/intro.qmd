...
The next most important rule is that rulings from the GM take precedent over the rules in this book. It's impossible for this book to offer rules and guidance on literally every situation that can occur at the table- so when a situation that isn't explained in this book crops up, the GM should rule based on what makes narrative (or at the very least, common) sense. See rule number one; the primary goal of the game is not to deconstruct the rules like it's the bar exam, but to tell a fulfilling story and have fun; and it's the GM's responsibility to make rulings to support that.
...
-   **The Character Sheet:** The character sheet is where each player will note down information pertaining to their character, and is continuously updated during play. A template character sheet is included with this book (TODO).

    - **The Inventory Sheet:** In World of PKMN, the player characters share an inventory of useful items when they are close together. This shared inventory is recorded as its own seperate sheet that all of the players are privy too. This is as simple as maintaining a list of items (and indicating if any items are Held by a particular team member); nevertheless, a template inventory sheet is also included with this book.
